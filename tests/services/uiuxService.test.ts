...
describe('UIUXService — Issue 1: Back button history guard', () => {
  ...
  beforeEach(() => {
    ...
    mockHistoryLength = window.history.length;
    originalPushState = window.history.pushState;
    window.history.pushState = jest.fn((state, title, url) => {
        mockHistoryLength++;
    });

    // Mock history.length property
  ...
  it('does NOT accumulate history entries on repeated updateSettings() calls', () => {
    service.updateSettings({ backButtonClosesNote: true });
    const afterFirstEnable = window.history.length;
    ...
    expect(window.history.length).toBe(afterFirstEnable);
  });
  ...
  it('resets backButtonInitialized when feature is disabled, allowing re-initialization', () => {
    service.updateSettings({ backButtonClosesNote: true });
    const afterEnable = window.history.length;
    ...
    service.updateSettings({ backButtonClosesNote: true });
    expect(window.history.length).toBe(afterEnable + 1);
  });
