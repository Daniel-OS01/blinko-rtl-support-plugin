...
## 2026-06-11 - Optimize NodeList Iteration
**Learning:** Iterating over `NodeList` objects (from `querySelectorAll`) directly via `for...of` is significantly faster and more memory-efficient than converting them to an array with `Array.from()` during frequent operations like event handling.
**Action:** Always prefer direct `for...of` iteration over `NodeList` objects instead of creating unnecessary array allocations.
