...
## 2026-04-27 - Optimize hot path text iteration with charCodeAt
**Learning:** In high-frequency text processing loops, allocating substrings, using regex `.test()` per character, and using Array iteration methods (`some()`) introduce significant allocation and boundary-crossing overhead.
**Action:** Use a standard `for` loop with `charCodeAt()` for direct integer evaluation to bypass object creation and massively improve execution speed, while handling UTF-16 surrogate pairs properly.
