...
## 2024-05-19 - Optimize Element Matching inside MutationObserver
**Learning:** Iterating over an array of CSS selectors and calling `element.matches(selector)` individually inside a high-frequency `MutationObserver` callback loop causes significant CPU overhead.
**Action:** Combine valid selectors into a single comma-separated string (e.g., `selectors.join(', ')`) outside the callback and evaluate it with a single `element.matches(joinedSelectors)` call to leverage native browser optimizations. Always ensure the joined string is not empty before calling `matches()`.
