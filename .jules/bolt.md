...
## 2024-07-10 - Optimize NodeList Iteration
**Learning:** Iterating over `NodeList` objects returned by `document.querySelectorAll` using `Array.from()` creates unnecessary memory allocations and adds overhead.
**Action:** Replace `Array.from(nodeList)` with direct `for...of` loops over the `NodeList` to reduce memory allocations and improve parsing performance, especially in hot paths. Use traditional control flow (`break`/`return`) where `.find()` or `.filter()` would have been used.
