...
## 2024-05-19 - Optimize High-Frequency Node Matching
**Learning:** In high-frequency DOM manipulation paths (like `MutationObserver` callbacks), checking `element.matches(selector)` within a loop over multiple CSS selectors causes significant CPU overhead because it prevents the browser from utilizing native optimization techniques for combined selector evaluation.
**Action:** Combine valid CSS selectors into a single comma-separated string (e.g., `selectors.join(', ')`) and evaluate it using a single `.matches(joinedSelectors)` call to leverage native browser optimizations and vastly improve parsing speed.
