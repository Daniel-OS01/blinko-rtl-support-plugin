...
## 2026-07-01 - Optimize MutationObserver selector matching with combined selectors
**Learning:** Iterating over an array of CSS selectors and calling `.matches()` individually inside a high-frequency `MutationObserver` loop incurs massive overhead due to repeated JavaScript-to-C++ boundary crossings.
**Action:** Use a single `element.matches(joinedSelectors)` call with a comma-separated string of valid selectors instead of looping. Always ensure the joined string is not empty before calling to avoid DOM exceptions.
