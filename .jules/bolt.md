...
## 2026-05-15 - [DOM Optimization]
**Learning:** Using `element.matches(joinedSelectors)` is significantly faster than looping through an array of selectors and calling `element.matches(s)` for each one, especially in high-frequency paths like a `MutationObserver`.
**Action:** When evaluating multiple CSS selectors against an element, join the valid selectors into a single comma-separated string and perform one `matches()` check.
