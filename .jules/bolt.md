...
## 2024-05-19 - Optimize Text Processing with charCodeAt
**Learning:** Using regex `.test()`, higher-order array closures like `.some()`, and string slicing like `.substring()` inside hot text parsing loops introduces significant execution overhead and memory allocation.
**Action:** Replace them with limit calculations (e.g., `Math.min()`), direct character code integer bounds checking (e.g., `charCodeAt`), and flat `for` loops for measurable speedups in frequent parsing paths.
