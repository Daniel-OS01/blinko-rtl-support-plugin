...
## 2026-06-21 - Optimize NodeList Iteration
**Learning:** Iterating over `NodeList` objects (from `querySelectorAll`) directly via `for...of` is significantly faster than converting them to an array with `Array.from()`, as it avoids unnecessary memory allocations and reduces iteration overhead.
**Action:** Replace `Array.from(nodeList)` with direct `for...of` loops, especially when the iteration can short-circuit early (like a manual `.find()`), to improve rendering performance and reduce garbage collection pressure.
