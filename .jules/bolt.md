...
## 2026-06-25 - Optimize NodeList Iteration
**Learning:** Iterating over `NodeList` objects (from `querySelectorAll`) via `Array.from()` is an anti-pattern as it introduces unnecessary memory allocations and array traversals. Converting an array-like structure solely to use `.find()` is a common performance pitfall.
**Action:** Direct `for...of` loops on `NodeList` with traditional `break` or `return` conditions should always be preferred over `Array.from(nodeList)` to minimize overhead and allow for early iteration exits.
