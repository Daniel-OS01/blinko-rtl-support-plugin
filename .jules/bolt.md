...
## 2026-05-01 - Optimize Selector Matching
**Learning:** Replacing loop-based `.matches()` calls with a single combined comma-separated selector string speeds up high-frequency MutationObserver callbacks.
**Action:** Use a joined selector string for `matches()` with a loop fallback for safety.
