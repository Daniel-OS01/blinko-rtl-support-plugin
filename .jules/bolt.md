...
## 2026-05-10 - Optimize RTL Detection Character Processing
**Learning:** In high-frequency text processing like RTL detection, using `for...of` string iterators, allocating substrings, array `.some()`, and `RegExp.test()` creates massive overhead due to object allocations and execution engine costs.
**Action:** Replaced these with direct `.charCodeAt()` integer evaluations and manual boundary checking to bypass object creation overhead, drastically improving execution speed while processing characters.
