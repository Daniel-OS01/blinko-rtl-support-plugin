      ...
      <div style={{ 
        ...
        <div style={{ marginBottom: '15px', padding: '15px', background: settings.darkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)', borderRadius: '6px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button
              onClick={(e) => {
                if (!settings.enabled || !selectedPresetId) { e.preventDefault(); return; }
                loadPreset();
              }}
              aria-disabled={!settings.enabled || !selectedPresetId}
              title={!settings.enabled ? "Enable RTL settings to use this feature" : !selectedPresetId ? "Select a preset to load" : "Load preset"}
              style={{
                background: '#17a2b8',
                color: 'white',
                border: 'none',
                ...
                opacity: (!settings.enabled || !selectedPresetId) ? 0.5 : 1
              }}
            >
            ...
            <button
              onClick={(e) => {
                if (!settings.enabled || !selectedPresetId || BUILT_IN_PRESETS.some(p => p.id === selectedPresetId)) { e.preventDefault(); return; }
                deletePreset();
              }}
              aria-disabled={!settings.enabled || !selectedPresetId || BUILT_IN_PRESETS.some(p => p.id === selectedPresetId)}
              aria-label="Delete selected preset"
              title={!settings.enabled ? "Enable RTL settings to use this feature" : BUILT_IN_PRESETS.some(p => p.id === selectedPresetId) ? "Built-in presets cannot be deleted" : "Delete selected preset"}
              style={{
                background: '#dc3545',
                color: 'white',
                border: 'none',
