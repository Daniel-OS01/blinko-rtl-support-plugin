      ...
      <div style={{ 
        ...
        <div style={{ marginBottom: '15px', padding: '15px', background: settings.darkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)', borderRadius: '6px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {(() => {
              const isDeleteDisabled = !settings.enabled || !selectedPresetId || BUILT_IN_PRESETS.some(p => p.id === selectedPresetId);
              return (
                <button
                  onClick={(e) => {
                    if (isDeleteDisabled) {
                  ...
                  title={(!settings.enabled || !selectedPresetId) ? "Select a preset to delete" : (BUILT_IN_PRESETS.some(p => p.id === selectedPresetId) ? "Cannot delete built-in presets" : "Delete selected preset")}
                >
