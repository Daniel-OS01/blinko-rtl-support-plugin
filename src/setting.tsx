        ...
        <div style={{ marginBottom: '15px', padding: '15px', background: settings.darkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)', borderRadius: '6px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button
              ...
              title={!settings.enabled ? "Enable plugin to load presets" : !selectedPresetId ? "Select a preset to load" : "Load selected preset"}
              style={{
                background: '#17a2b8',
                color: 'white',
                border: 'none',
            ...
            <button
              onClick={(e) => {
                const isBuiltIn = BUILT_IN_PRESETS.some(p => p.id === selectedPresetId);
                if (!settings.enabled || !selectedPresetId || isBuiltIn) {
                  e.preventDefault();
                  return;
                }
                deletePreset();
              ...
              title={
                !settings.enabled ? "Enable plugin to delete presets" :
                !selectedPresetId ? "Select a preset to delete" :
                BUILT_IN_PRESETS.some(p => p.id === selectedPresetId) ? "Built-in presets cannot be deleted" :
