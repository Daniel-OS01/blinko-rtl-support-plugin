...
export class PasteInterceptor {
  ...
  private detectMixedContent(text: string): boolean {
    ...
    // Fast path: check characters directly and exit early
    for (let i = 0; i < text.length; i++) {
      const code = text.charCodeAt(i);

      // Check for RTL chars: Hebrew (0x0590-0x05FF) or Arabic (0x0600-0x06FF)
      if ((code >= 0x0590 && code <= 0x05FF) || (code >= 0x0600 && code <= 0x06FF)) {
        rtlCount++;
      }
      // Check for LTR chars: A-Z (0x0041-0x005A) or a-z (0x0061-0x007A)
