...
export class CharacterCodeStrategy implements DetectionStrategy {
  ...
  /**
   * Fast integer-based check if a character code falls into an RTL unicode block
   */
  ...
  public detect(text: string): boolean {
    ...
    for (let i = 0; i < len; i++) {
      ...
      // Fast path for ASCII whitespace and common punctuation (up to ~code 125 '}')
      // Matches original behavior of /\\s|[.,!?;:()[\\]{}]/ roughly
      if (code <= 125) {
        if (
          code === 32 || code === 9 || code === 10 || code === 13 || // Common whitespace
      ...
      // Check for remaining whitespace like NBSP that `\s` originally caught
      if (code === 160) {
        continue;
      }
