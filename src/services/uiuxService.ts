...
export class UIUXService {
  ...
  private applyBackButton(): void {
    ...
    const runBackButtonHandler = (_e: PopStateEvent) => {
      ...
      const findVisibleOverlay = (): HTMLElement | null => {
        ...
        // 💡 What: Use direct iteration over NodeList instead of Array.from()
        // 🎯 Why: Avoids unnecessary array allocation; direct iteration is significantly faster.
      ...
      if (overlay) {
        ...
        // 💡 What: Replaced Array.from().find() with a direct for...of loop over NodeList.
        // 🎯 Why: Prevents array allocation overhead and allows an immediate early exit.
        const closeBtns = overlay.querySelectorAll<HTMLElement>('button, [class*="close"], [aria-label], [data-dismiss]');
  ...
  private applyTapOutsideClose(): void {
    ...
    const findActiveOverlay = (): HTMLElement | null => {
      ...
      // 💡 What: Direct for...of iteration over NodeList.
      // 🎯 Why: Avoids unnecessary Array.from() allocations and improves performance.
      for (const el of candidates) {
        if (el.style.display === 'none' || el.style.visibility === 'hidden') continue;
        return el;
      }
    ...
    const closeViaButtonOrEscape = (scope: HTMLElement): void => {
      // 💡 What: Replaced Array.from().find() with a direct for...of loop over NodeList.
      // 🎯 Why: Avoids memory allocation overhead from Array.from() and allows immediate early exit.
      const closeBtns = scope.querySelectorAll<HTMLElement>('button, [class*="close"], [aria-label], [data-dismiss]');
      let closeBtn: HTMLElement | null = null;
      for (const el of closeBtns) {
        const ariaLabel = (el.getAttribute('aria-label') || '').toLowerCase();
        const className = (el.className || '').toString().toLowerCase();
