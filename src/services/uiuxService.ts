...
export class UIUXService {
  ...
  private applyBackButton(): void {
    ...
    const runBackButtonHandler = (_e: PopStateEvent) => {
      ...
      if (overlay) {
        ...
        // Array.from is avoided here since iterating over a NodeList is much faster.
        let closeBtn: HTMLElement | null = null;
        const potentialCloseBtns = overlay.querySelectorAll<HTMLElement>('button, [class*="close"], [aria-label], [data-dismiss]');
        for (const el of potentialCloseBtns) {
  ...
  private applyTapOutsideClose(): void {
    ...
    const closeViaButtonOrEscape = (scope: HTMLElement): void => {
      // Array.from is avoided here since iterating over a NodeList is much faster.
      let closeBtn: HTMLElement | null = null;
      const potentialCloseBtns = scope.querySelectorAll<HTMLElement>('button, [class*="close"], [aria-label], [data-dismiss]');
      for (const el of potentialCloseBtns) {
        const ariaLabel = (el.getAttribute('aria-label') || '').toLowerCase();
        const className = (el.className || '').toString().toLowerCase();
