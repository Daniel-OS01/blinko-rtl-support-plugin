...
export class UIUXService {
  ...
  private applyBackButton(): void {
    ...
    const runBackButtonHandler = (_e: PopStateEvent) => {
      ...
      const findVisibleOverlay = (): HTMLElement | null => {
        ...
        // 🎯 Why: Iterating NodeList directly avoids allocating a new Array object
        // 📊 Impact: Micro-optimization, saves memory allocation overhead
      ...
      if (overlay) {
        ...
        const buttons = overlay.querySelectorAll<HTMLElement>('button, [class*="close"], [aria-label], [data-dismiss]');
        // 💡 What: Replaced Array.from().find() with direct loop
        // 🎯 Why: Iterating NodeList directly avoids Array allocation and allows early exit
        // 📊 Impact: O(n) iteration with early break, no intermediate O(n) space allocation
  ...
  private applyTapOutsideClose(): void {
    ...
    const closeViaButtonOrEscape = (scope: HTMLElement): void => {
      let closeBtn: HTMLElement | null = null;
      const buttons = scope.querySelectorAll<HTMLElement>('button, [class*="close"], [aria-label], [data-dismiss]');
      // 💡 What: Replaced Array.from().find() with direct loop
      // 🎯 Why: Iterating NodeList directly avoids Array allocation and allows early exit
      // 📊 Impact: O(n) iteration with early break, no intermediate O(n) space allocation
      for (const el of buttons) {
