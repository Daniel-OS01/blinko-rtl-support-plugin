...
.rtl-debug-ltr::after {
  ...
  private setupObserver() {
      ...
      // ⚡ Bolt: Compute active selectors and validate them OUTSIDE the MutationObserver callback.
      // Caching configuration-dependent calculations and valid selectors outside the mutation loop
      // avoids redundant array filtering and expensive DOM queries (`document.querySelector`) on every mutation.
      const activeSelectors = this.settings.targetSelectors.filter(
        s => !this.settings.disabledSelectors.includes(s)
      );
      ...
      activeSelectors.forEach(s => {
          try {
              document.querySelector(s); // Test selector validity
